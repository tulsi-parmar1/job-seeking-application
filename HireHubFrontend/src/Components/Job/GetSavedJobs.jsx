    ...
    useEffect(()=>{
        const getSavedJobs=async()=>{
            try {
                ...
                const {data}=await axios.get('http://localhost:4000/api/user/getSavedJobs',{withCredentials:true});
                        ...
                        { 
                          savedJobs.map(elemnt => {
                                return <div className="card" key={elemnt._id}>
                                    ...
                                    <CiHeart className={users && users.savedJobs && users.savedJobs.includes(elemnt._id) ? 'saved' : ''} />
                                    </span>
                                    <p>{elemnt.company}</p>
                                    <p>{elemnt.location}</p>
